...
'온라인 비대면 교육 퀴즈 참여가능기간 여부 체크
year_now = CStr(Year(Now()))

' sql = ""
' sql = sql & " SELECT Edu_Year, Emp_num, Edu_From, Edu_To "
' sql = sql & " 	,MenuYN = CASE WHEN CONVERT(CHAR(8), getdate(), 112) between Edu_From and Edu_To then 'Y' else 'N' end "
' sql = sql & " FROM Edu_Emp_List_TBL "
' sql = sql & " WHERE Edu_Year = " & year_now & " AND Emp_Num = '" & db_id & "' "
...
function WebPartToggle(obj)
{
	...
	function D_Appointment(Pstr)
	{
		...
		parent.frames[2].location = "right_Appointment.asp?Qstr=" + Pstr;
	}
	
	function D_EDU(Pstr)
	{
		parent.frames[2].location = "right_Edu_List.asp?Qstr=" + Pstr;
	}
	
	function D_EDU_Status(Pstr)
	{
		parent.frames[2].location = "right_Edu_Status.asp?Qstr=" + Pstr;
	}
</script>
...
<div id="left">
		...
		<ul>		     
					...
					<li><span style="width:100%" onClick="D_fUN('094','확인서')">확인서</span></li>
					<li><span style="width:100%" onClick="D_fUN('092','유사사례')">유사사례</span></li>
					<!--<li><span style="width:100%" onClick="D_fUN('093','일일수감')">일일수감</span></li>-->
					<li><span style="width:100%" onClick="D_Appointment('지킴약속10')">지킴약속10</span></li>
		</ul> 			
		<% 'end if %>
		...
		<h2>CM업무</h2>
		<ul>		     
					<li><span style="width:100%" onClick="D_fUN('021','CM양식')">CM양식</span></li>
		...
		<h2>감리업무(해외)</h2>
		<ul>		     
			<li><span style="width:100%" onClick="D_fUN('500','Supervision Manual and Inspection Check list(감리 매뉴얼과 검측 체크리스트)')">Supervision Manual and Inspection Check list(감리 매뉴얼과 검측 체크리스트)</span></li>
			<li><span style="width:100%" onClick="D_fUN('501','Reports(보고서)')">Reports(보고서)</span></li>
			<li><span style="width:100%" onClick="D_fUN('502','Interim Payment Certificates(기성 서류)')">Interim Payment Certificates(기성 서류)</span></li>
			<li><span style="width:100%" onClick="D_fUN('503','Contract Variation Order and Amendments(계약설계변경)')">Contract Variation Order and Amendments(계약설계변경)</span></li>
			<li><span style="width:100%" onClick="D_fUN('504','Completion Documents(준공서류)')">Completion Documents(준공서류)</span></li>						
		</ul> 	
		...
		<ul>		     
					...
					<li><span style="width:100%" onClick="D_fUN('042','공사/CM')">공사/CM</span></li>
					<li><span style="width:100%" onClick="D_fUN('043','설계도서')">설계도서</span></li>
					<li><span style="width:100%" onClick="D_fUN('044','CM사례')">CM사례</span></li>
		...
		<h2>비대면 온라인 교육</h2>
			<ul>				
				<li><span style="width:100%" onClick="D_EDU('비대면 온라인 교육 이수')">교육 이수</span></li>
				<li><span style="width:100%" onClick="D_EDU_Status('비대면 온라인 교육 이수 현황')">교육 이수 현황</span></li>
			</ul>				
