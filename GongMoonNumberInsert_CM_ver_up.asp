...
Resident_chef = "" '공문발송함(대표이사->발주청) 메뉴에서 상주 책임이 공문번호 생성한 경우 Y, 그 외 해당 변수 값 없음(23.01.19 경익수 상무 요청) => 이 페이지에선 해당 변수 값 들어올 일 없음
...
function PrintFrm(){	
	var CPage = "<%=Page%>"	
	var Ctype = "<%=type__%>"
	parent.frames[2].location = "right_main_gongmoon_insert_CM_ver_up.asp?Page=" + CPage + "&type__=" + Ctype ; // 발송함에서 기안자가 직접 따는 것으로 변경(2017.11.24/경익수 상무 요청)
}
</script>
