...
function read_gongMoon(o_seq, o_receive_id, o_send_longname, o_visited, private__) {
	...
	url = "../ezEmail/mail_read_ver_up.asp?Seq=" + o_seq + "&rID=" + o_receive_id + "&stype=접수&HJname=" + o_send_longname + "&visited=" + o_visited + "&PP=" + private__;
			...
			<table width="100%" class="popuplist">
				...
				%>
							<tr>
								...
								<td style="text-align:center;"><%=rs("pm")%></td>
								<td style="text-align:right;"><%=formatnumber(DbRec("receive_cnt"),0,-1,0,-1)%></td>
								<td style="text-align:right;"><%=formatnumber(DbRec("read_cnt"),0,-1,0,-1)%></td>
								<td style="text-align:right;"><%=formatnumber(DbRec("unread_cnt"),0,-1,0,-1)%></td>
								<td style="text-align:right;"><%=formatnumber(DbRec("appr_cnt"),0,-1,0,-1)%></td>
							</tr>
				<%		 	end if 
							set DbRec = Nothing 
						rs.MoveNext
